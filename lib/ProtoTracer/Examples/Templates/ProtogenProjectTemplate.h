 ...
 */
 ...
 #include "BlushSequence.h"
 #include "BombSequence.h"
 #include "ApertureSequence.h"
 ...
 private:
     ...
     Vector2D BlushSize = Vector2D(128, 128);
     Vector2D BlushOffset = Vector2D(90, 50);
     
 
     Vector2D BombSize = Vector2D(128, 128);
     Vector3D BombOffset = Vector3D(75, 50, 9999999999999);

     Vector2D ApertureSize = Vector2D(100, 100);
     Vector2D ApertureOffset = Vector2D(120, 80);
     ...
     SimpleMaterial purpleMaterial = SimpleMaterial(RGBColor(70, 0, 211)); ///< Solid purple material.
     SimpleMaterial blackMaterial  = SimpleMaterial(RGBColor(245, 17, 161));     ///< Solid black material.
 ...
 protected:
     ...
     * @param mirror Whether to mirror along the X-axis.
     ...
     BlushSequence blushsequence = BlushSequence(BlushSize, BlushOffset, 12.5f);
     BombSequence bombsequence = BombSequence(BombSize, BombOffset, 15.0f);
     ApertureSequence aperturesequence = ApertureSequence(ApertureSize, ApertureOffset, 11.0f);
 ...
 public:
     ...
     /**
      * @brief Sets the background material to the specified material.
      * @param material Pointer to the new material to set as the background.
      */
