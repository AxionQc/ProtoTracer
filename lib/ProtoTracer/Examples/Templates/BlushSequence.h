...
class BlushSequence : public ImageSequenceRGB{
private:
	...
	Image image = Image(frame0000, rgbColors0000, 152, 152, 16);

public:
	BlushSequence(Vector2D size, Vector2D offset, float fps) : ImageSequenceRGB(&image, sequence, RGBsequence, (unsigned int)25, fps) {
		image.SetSize(size);
