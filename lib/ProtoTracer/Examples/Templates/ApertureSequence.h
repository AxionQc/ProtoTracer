...
class ApertureSequence : public ImageSequenceRGB{
private:
	...
	static const uint8_t frame0025[];
	static const uint8_t frame0026[];
	...
	static const uint8_t rgbColors0025[];
	static const uint8_t rgbColors0026[];

	static const uint8_t* sequence[27];


	static const uint8_t* RGBsequence[27];

	Image image = Image(frame0000, rgbColors0000, 128, 128, 256);

public:
	ApertureSequence(Vector2D size, Vector2D offset, float fps) : ImageSequenceRGB(&image, sequence, RGBsequence, (unsigned int)27, fps) {
		image.SetSize(size);
